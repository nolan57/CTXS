...
namespace CTXS
{
    public partial class CTXSRibbon
    {
        ...
        private void CTXSButton_Click(object sender, RibbonControlEventArgs e)
        {
            ...
            for(long R= 4;R<LastRow;R++)
            {
                if (Convert.ToString(Worksheet.Cells[R, 4].value).Substring(0, 5).Equals(TireKeyNum, StringComparison.OrdinalIgnoreCase))
                {
                    PartNuStartCell = Worksheet.Cells[R, 4];
                    PartNameStartCell = Worksheet.Cells[R, 6];
                    PSStartCell = Worksheet.Cells[R, 24];
